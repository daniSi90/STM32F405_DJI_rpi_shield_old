#ifndef TR_ONE_H
#define TR_ONE_H 100

/* C++ detection */
#ifdef __cplusplus
extern "C" {
#endif


#include "main.h"
	


/**
 * @brief  Data rates predefined constants
 * @{
 */
#define TRONE_ADD       	0   	 /* Default base address */
#define TRONE_WHO_AM_I		0x01   // Who am i
#define TRONE_CHA_ADDR		0xA2   // Change base address
#define TRONE_I_AM        0xA1

/**
 * @brief  TeraRanger One result enumeration	
 */
typedef enum _TERAONE_Result {
	TERAONE_OK       = 0x00U,
  TERAONE_ERROR    = 0x01U,
} TERAONE_Result;


/**
 * @brief  Main TeraRanger One structure
 */
typedef struct _TRONE_Str {
	/* Private */
	I2C_HandleTypeDef* 	i2cHandle;
	uint8_t 						Address;         /*!< I2C address of device. Only for private use */
	uint16_t 						distance;         /*!< Gyroscope corrector from raw data to "degrees/s". Only for private use */
} TRONE_Str;

// Create a Cyclic Redundancy Checks table used in the "crc8" function
static const uint8_t crc_table[] = {
    
};

TERAONE_Result TrOne_WhoAmI(TRONE_Str *senStruct);
TERAONE_Result TrOne_ReadDist(TRONE_Str *senStruct);
TERAONE_Result TrOne_ChangeBaseAddr(TRONE_Str *senStruct); // primoz
uint8_t TrOne_crc8(uint8_t *p, uint8_t len);

/* C++ detection */
#ifdef __cplusplus
}
#endif

#endif

